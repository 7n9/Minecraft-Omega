...
public class SaveFormatOld implements ISaveFormat {
	...
	public List func_22176_b() {
		...
		for(int i2 = 0; i2 < 5; ++i2) {
			...
			if(worldInfo4 != null) {
				arrayList1.add(new SaveFormatComparator(string3, "", worldInfo4.getLastTimePlayed(), worldInfo4.getSizeOnDisk(), worldInfo4.func_35918_q(), false));
			}
		}
	...
	public WorldInfo getWorldInfo(String string1) {
		...
		if(!file2.exists()) {
			...
			if(file3.exists()) {
				try {
					nBTTagCompound4 = CompressedStreamTools.loadGzippedCompoundFromOutputStream(new FileInputStream(file3));
	...
	public void func_22170_a(String string1, String string2) {
		...
		if(file3.exists()) {
			...
			if(file4.exists()) {
				try {
					NBTTagCompound nBTTagCompound5 = CompressedStreamTools.loadGzippedCompoundFromOutputStream(new FileInputStream(file4));
