...
public class CanvasIsomPreview extends Canvas implements KeyListener, MouseListener, MouseMotionListener, Runnable {
	...
	public void loadWorld(String string1) {
		...
		this.worldObj = new World(new SaveHandler(new File(this.dataFolder, "saves"), string1, false), string1, new WorldSettings((new Random()).nextLong(), 0, true));
		...
		synchronized(this.imageBufferList) {
			...
			for(int i3 = 0; i3 < 64; ++i3) {
				for(int i4 = 0; i4 < 64; ++i4) {
					this.imageBuffers[i3][i4].setWorldAndChunkPosition(this.worldObj, i3, i4);
				}
			}
	...
	private IsoImageBuffer getImageBuffer(int i1, int i2) {
		...
		if(isoImageBuffer5.chunkX == i1 && isoImageBuffer5.chunkZ == i2) {
			...
			isoImageBuffer5.setChunkPosition(i1, i2);
			return isoImageBuffer5;
		}
	}
	...
	public void drawScreen(Graphics2D graphics2D1) {
		...
		int i10000 = rectangle17.y + rectangle17.height + 16;
		this.worldObj.getClass();
		int i9 = (i10000 + 128) / b4 + 1 + b5 * 2;
