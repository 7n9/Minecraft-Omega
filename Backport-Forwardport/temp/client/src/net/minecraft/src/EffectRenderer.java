...
public class EffectRenderer {
		...
		for(int i8 = 0; i8 < 3; ++i8) {
			if(this.fxLayers[i8].size() != 0) {
				...
				for(int i11 = 0; i11 < this.fxLayers[i8].size(); ++i11) {
					...
					tessellator10.func_35835_b(entityFX12.func_35115_a(f2));
	...
	public void func_1187_b(Entity entity1, float f2) {
		...
		float f7 = MathHelper.cos(entity1.rotationPitch * (float)Math.PI / 180.0F);
		byte b8 = 3;
		if(this.fxLayers[b8].size() != 0) {
			Tessellator tessellator9 = Tessellator.instance;

			for(int i10 = 0; i10 < this.fxLayers[b8].size(); ++i10) {
				EntityFX entityFX11 = (EntityFX)this.fxLayers[b8].get(i10);
				tessellator9.func_35835_b(entityFX11.func_35115_a(f2));
				entityFX11.renderParticle(tessellator9, f2, f3, f7, f4, f5, f6);
			}
