...
public class MapData extends MapDataBase {
	...
	public void func_28155_a(EntityPlayer entityPlayer1, ItemStack itemStack2) {
		...
		for(int i14 = 0; i14 < this.field_28158_h.size(); ++i14) {
			...
			if(!mapInfo4.entityplayerObj.isDead && mapInfo4.entityplayerObj.inventory.func_28010_c(itemStack2)) {
				float f5 = (float)(mapInfo4.entityplayerObj.posX - (double)this.field_28164_b) / (float)(1 << this.field_28161_e);
				float f6 = (float)(mapInfo4.entityplayerObj.posZ - (double)this.field_28163_c) / (float)(1 << this.field_28161_e);
				byte b7 = 64;
				byte b8 = 64;
				if(f5 >= (float)(-b7) && f6 >= (float)(-b8) && f5 <= (float)b7 && f6 <= (float)b8) {
					...
					if(mapInfo4.entityplayerObj.dimension == this.field_28162_d) {
