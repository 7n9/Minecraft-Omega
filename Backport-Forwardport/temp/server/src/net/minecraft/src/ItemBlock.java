...
public class ItemBlock extends Item {
	...
	public boolean onItemUse(ItemStack itemStack1, EntityPlayer entityPlayer2, World world3, int i4, int i5, int i6, int i7) {
		...
		if(i8 == Block.snow.blockID) {
			i7 = 0;
		} else if(i8 != Block.field_35045_bv.blockID) {
			if(i7 == 0) {
				--i5;
			}
		...
		if(itemStack1.stackSize == 0) {
			return false;
		} else if(!entityPlayer2.func_35200_c(i4, i5, i6)) {
			return false;
		} else {
			world3.getClass();
			if(i5 == 128 - 1 && Block.blocksList[this.blockID].blockMaterial.isSolid()) {
				return false;
				...
				if(world3.setBlockAndMetadataWithNotify(i4, i5, i6, this.blockID, this.getMetadata(itemStack1.getItemDamage()))) {
					...
					world3.playSoundEffect((double)((float)i4 + 0.5F), (double)((float)i5 + 0.5F), (double)((float)i6 + 0.5F), block9.stepSound.stepSoundDir(), (block9.stepSound.getVolume() + 1.0F) / 2.0F, block9.stepSound.getPitch() * 0.8F);
