...
public class MinecraftServer implements Runnable, ICommandListener {
	...
	private boolean startServer() throws UnknownHostException {
		...
		logger.info("Starting minecraft server version Beta 1.8");
	...
	private void initWorld(ISaveFormat iSaveFormat1, String string2, long j3) {
		...
		int i5 = this.propertyManagerObj.getIntProperty("gamemode", 0);
		i5 = WorldSettings.func_35037_a(i5);
		logger.info("Default game type: " + i5);
		WorldSettings worldSettings6 = new WorldSettings(j3, i5, true);
		SaveOldDir saveOldDir7 = new SaveOldDir(new File("."), string2, true);

		for(int i8 = 0; i8 < this.worldMngr.length; ++i8) {
			if(i8 == 0) {
				this.worldMngr[i8] = new WorldServer(this, saveOldDir7, string2, i8 == 0 ? 0 : -1, worldSettings6);
			} else {
				this.worldMngr[i8] = new WorldServerMulti(this, saveOldDir7, string2, i8 == 0 ? 0 : -1, worldSettings6, this.worldMngr[0]);
			}

			this.worldMngr[i8].addWorldAccess(new WorldManager(this, this.worldMngr[i8]));
			this.worldMngr[i8].difficultySetting = this.propertyManagerObj.getIntProperty("difficulty", 1);
			this.worldMngr[i8].setAllowedSpawnTypes(this.propertyManagerObj.getBooleanProperty("spawn-monsters", true), this.spawnPeacefulMobs);
			this.worldMngr[i8].getWorldInfo().func_35500_d(i5);
		...
		for(int i11 = 0; i11 < this.worldMngr.length; ++i11) {
			logger.info("Preparing start region for level " + i11);
			if(i11 == 0 || this.propertyManagerObj.getBooleanProperty("allow-nether", true)) {
				WorldServer worldServer12 = this.worldMngr[i11];
				ChunkCoordinates chunkCoordinates13 = worldServer12.getSpawnPoint();

				for(int i14 = -s20; i14 <= s20 && this.serverRunning; i14 += 16) {
					for(int i15 = -s20; i15 <= s20 && this.serverRunning; i15 += 16) {
						long j16 = System.currentTimeMillis();
						if(j16 < j9) {
							j9 = j16;
						}

						if(j16 > j9 + 1000L) {
							int i18 = (s20 * 2 + 1) * (s20 * 2 + 1);
							int i19 = (i14 + s20) * (s20 * 2 + 1) + i15 + 1;
							this.outputPercentRemaining("Preparing spawn area", i19 * 100 / i18);
							j9 = j16;
						}

						worldServer12.chunkProviderServer.loadChunk(chunkCoordinates13.posX + i14 >> 4, chunkCoordinates13.posZ + i15 >> 4);

						while(worldServer12.updatingLighting() && this.serverRunning) {
						}
					}
				}
			}
