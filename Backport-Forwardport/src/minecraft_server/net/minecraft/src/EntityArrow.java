		...
		if(this.inGround) {
			...
			if(movingObjectPosition3 != null) {
				if(movingObjectPosition3.entityHit != null) {
					...
					int i20 = (int)Math.ceil((double)f19 * 2.0D);
					if(this.field_35160_d) {
						i20 = i20 * 3 / 2 + 1;
					}

					if(movingObjectPosition3.entityHit.attackEntityFrom(DamageSource.func_35073_a(this, this.shootingEntity), i20)) {
						if(movingObjectPosition3.entityHit instanceof EntityLiving) {
							++((EntityLiving)movingObjectPosition3.entityHit).field_35189_aD;
						}
