...
public class SaveConverterMcRegion extends SaveFormatOld {
		...
		for(int i5 = 0; i5 < i4; ++i5) {
			...
			if(file6.isDirectory()) {
				...
				if(worldInfo8 != null) {
					...
					arrayList1.add(new SaveFormatComparator(string7, string10, worldInfo8.getLastTimePlayed(), worldInfo8.getSizeOnDisk(), worldInfo8.func_35918_q(), z9));
				}
			}
		}
	...
	private void func_22181_a(File file1, ArrayList arrayList2, int i3, int i4, IProgressUpdate iProgressUpdate5) {
		...
		while(iterator7.hasNext()) {
			...
			int i9 = chunkFile8.getXChunk();
			int i10 = chunkFile8.getYChunk();
			RegionFile regionFile11 = RegionFileCache.createOrLoadRegionFile(file1, i9, i10);
			if(!regionFile11.isChunkSaved(i9 & 31, i10 & 31)) {
				try {
					DataInputStream dataInputStream12 = new DataInputStream(new GZIPInputStream(new FileInputStream(chunkFile8.getChunkFile())));
