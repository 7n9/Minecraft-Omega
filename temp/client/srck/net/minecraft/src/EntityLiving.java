...
public abstract class EntityLiving extends Entity {
	...
	public void onUpdate() {
		...
		if(f5 > 0.05F) {
			f8 = 1.0F;
			f7 = f5 * 3.0F;
			f6 = (float)Math.atan2(d3, d1) * 180.0F / 3.1415927F - 90.0F;
		}
			...
			if(this.health <= 0) {
				...
				if(z3) {
					...
					if(entity1 != null) {
						...
						this.attackedAtYaw = (float)(Math.atan2(d6, d4) * 180.0D / 3.1415927410125732D) - this.rotationYaw;
	...
	protected void fall(float f1) {
		...
		if(i2 > 0) {
			...
			int i3 = this.worldObj.getBlockId(MathHelper.floor_double(this.posX), MathHelper.floor_double(this.posY - 0.20000000298023224D - (double)this.yOffset), MathHelper.floor_double(this.posZ));
			if(i3 > 0) {
	...
	public void moveEntityWithHeading(float f1, float f2) {
		...
		if(this.isInWater()) {
			...
			this.motionY -= 0.02D;
			if(this.isCollidedHorizontally && this.isOffsetPositionInLiquid(this.motionX, this.motionY + 0.6000000238418579D - this.posY + d3, this.motionZ)) {
	...
	public void onLivingUpdate() {
		if(this.newPosRotationIncrements > 0) {
			...
			List list9 = this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.func_28195_e(0.03125D, 0.0D, 0.03125D));
			if(list9.size() > 0) {
				double d10 = 0.0D;
		...
		List list15 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(0.20000000298023224D, 0.0D, 0.20000000298023224D));
	...
	public void faceEntity(Entity entity1, float f2, float f3) {
		...
		float f12 = (float)(Math.atan2(d8, d4) * 180.0D / 3.1415927410125732D) - 90.0F;
		float f13 = (float)(-(Math.atan2(d6, d14) * 180.0D / 3.1415927410125732D));
	...
	public Vec3D getLook(float f1) {
		...
		float f5;
		if(f1 == 1.0F) {
			f2 = MathHelper.cos(-this.rotationYaw * 0.017453292F - 3.1415927F);
			f3 = MathHelper.sin(-this.rotationYaw * 0.017453292F - 3.1415927F);
			...
			f4 = MathHelper.cos(-f3 * 0.017453292F - 3.1415927F);
			f5 = MathHelper.sin(-f3 * 0.017453292F - 3.1415927F);
