...
public class RenderGlobal implements IWorldAccess {
	...
	private void renderStars() {
		...
		for(int i3 = 0; i3 < 1500; ++i3) {
			...
			if(d12 < 1.0D && d12 > 0.01D) {
				...
				double d32 = random1.nextDouble() * 3.141592653589793D * 2.0D;
					...
					if(this.sortedWorldRenderers[i23].skipAllRenderPasses()) {
						...
						if(this.sortedWorldRenderers[i23].isInFrustum && !this.sortedWorldRenderers[i23].isWaitingOnOcclusionQuery) {
							...
							if(this.cloudOffsetX % i25 == i23 % i25) {
								...
								ARBOcclusionQuery.glBeginQueryARB(35092, this.sortedWorldRenderers[i23].glOcclusionQuery);
		...
		if(!this.mc.theWorld.worldProvider.isNether) {
			...
			if(f18 != null) {
				...
				for(int i20 = 0; i20 <= b19; ++i20) {
					f14 = (float)i20 * 3.1415927F * 2.0F / (float)b19;
	...
	public void renderCloudsFancy(float f1) {
		...
		double d8 = (this.mc.renderViewEntity.prevPosZ + (this.mc.renderViewEntity.posZ - this.mc.renderViewEntity.prevPosZ) * (double)f1) / (double)f4 + 0.33000001311302185D;
