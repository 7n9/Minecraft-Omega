...
public abstract class WorldProvider {
	...
	public float calculateCelestialAngle(long j1, float f3) {
		...
		float f6 = f5;
		f5 = 1.0F - (float)((Math.cos((double)f5 * 3.141592653589793D) + 1.0D) / 2.0D);
		f5 = f6 + (f5 - f6) / 3.0F;
		return f5;
	}
	...
	public float[] calcSunriseSunsetColors(float f1, float f2) {
		float f3 = 0.4F;
		float f4 = MathHelper.cos(f1 * 3.1415927F * 2.0F) - 0.0F;
		float f5 = -0.0F;
		if(f4 >= f5 - f3 && f4 <= f5 + f3) {
			float f6 = (f4 - f5) / f3 * 0.5F + 0.5F;
			float f7 = 1.0F - (1.0F - MathHelper.sin(f6 * 3.1415927F)) * 0.99F;
			f7 *= f7;
	...
	public Vec3D func_4096_a(float f1, float f2) {
		float f3 = MathHelper.cos(f1 * 3.1415927F * 2.0F) * 2.0F + 0.5F;
		if(f3 < 0.0F) {
			f3 = 0.0F;
		}
