...
public class RenderFish extends Render {
	public void func_4011_a(EntityFish entityFish1, double d2, double d4, double d6, float f8, float f9) {
		...
		if(entityFish1.angler != null) {
			float f20 = (entityFish1.angler.prevRotationYaw + (entityFish1.angler.rotationYaw - entityFish1.angler.prevRotationYaw) * f9) * 3.1415927F / 180.0F;
			double d21 = (double)MathHelper.sin(f20);
			double d23 = (double)MathHelper.cos(f20);
			float f25 = entityFish1.angler.getSwingProgress(f9);
			float f26 = MathHelper.sin(MathHelper.sqrt_float(f25) * 3.1415927F);
			Vec3D vec3D27 = Vec3D.createVector(-0.5D, 0.03D, 0.8D);
			vec3D27.rotateAroundX(-(entityFish1.angler.prevRotationPitch + (entityFish1.angler.rotationPitch - entityFish1.angler.prevRotationPitch) * f9) * 3.1415927F / 180.0F);
			vec3D27.rotateAroundY(-(entityFish1.angler.prevRotationYaw + (entityFish1.angler.rotationYaw - entityFish1.angler.prevRotationYaw) * f9) * 3.1415927F / 180.0F);
			...
			if(this.renderManager.options.thirdPersonView) {
				f20 = (entityFish1.angler.prevRenderYawOffset + (entityFish1.angler.renderYawOffset - entityFish1.angler.prevRenderYawOffset) * f9) * 3.1415927F / 180.0F;
