...
public abstract class Entity {
	...
	public void onEntityUpdate() {
		...
		if(this.handleWaterMovement()) {
			if(!this.inWater && !this.isFirstUpdate) {
				float f1 = MathHelper.sqrt_double(this.motionX * this.motionX * 0.20000000298023224D + this.motionY * this.motionY + this.motionZ * this.motionZ * 0.20000000298023224D) * 0.2F;
				if(f1 > 1.0F) {
					f1 = 1.0F;
				}
	...
	public void moveEntity(double d1, double d3, double d5) {
		if(this.noClip) {
			...
			if(this.canTriggerWalking() && !z18 && this.ridingEntity == null) {
				...
				i26 = MathHelper.floor_double(this.posY - 0.20000000298023224D - (double)this.yOffset);
	...
	public void moveFlying(float f1, float f2, float f3) {
		...
		if(f4 >= 0.01F) {
			...
			f1 *= f4;
			f2 *= f4;
			float f5 = MathHelper.sin(this.rotationYaw * 3.1415927F / 180.0F);
			float f6 = MathHelper.cos(this.rotationYaw * 3.1415927F / 180.0F);
	...
	public void setPositionAndRotation2(double d1, double d3, double d5, float f7, float f8, int i9) {
		...
		this.setRotation(f7, f8);
		List list10 = this.worldObj.getCollidingBoundingBoxes(this, this.boundingBox.func_28195_e(0.03125D, 0.0D, 0.03125D));
		if(list10.size() > 0) {
			double d11 = 0.0D;
