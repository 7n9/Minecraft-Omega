...
public class EntityBoat extends Entity {
	...
	public void onUpdate() {
		...
		if(this.worldObj.multiplayerWorld) {
			...
			if(d2 < 1.0D) {
				d21 = d2 * 2.0D - 1.0D;
				this.motionY += 0.03999999910593033D * d21;
			} else {
			...
			if(d6 > 0.15D) {
				d8 = Math.cos((double)this.rotationYaw * 3.141592653589793D / 180.0D);
				d10 = Math.sin((double)this.rotationYaw * 3.141592653589793D / 180.0D);
			...
			if(d10 * d10 + d23 * d23 > 0.001D) {
				d8 = (double)((float)(Math.atan2(d23, d10) * 180.0D / 3.141592653589793D));
			}
			...
			this.setRotation(this.rotationYaw, this.rotationPitch);
			List list16 = this.worldObj.getEntitiesWithinAABBExcludingEntity(this, this.boundingBox.expand(0.20000000298023224D, 0.0D, 0.20000000298023224D));
			int i24;
	...
	public void updateRiderPosition() {
		if(this.riddenByEntity != null) {
			double d1 = Math.cos((double)this.rotationYaw * 3.141592653589793D / 180.0D) * 0.4D;
			double d3 = Math.sin((double)this.rotationYaw * 3.141592653589793D / 180.0D) * 0.4D;
